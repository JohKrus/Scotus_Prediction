...
from scotus_v2 import config, keys
...
def get_syllabus_models() -> dict[str, ChatOpenAI | ChatGoogleGenerativeAI | ChatAnthropic]:
    """LLMs configured for syllabus generation. Skips quota-blocked models."""
    api_keys = _ensure_env()
    cfg = config.load()
    model_cfg = cfg["models"]
    syl = cfg["syllabus"]
    ...
    all_models = {
        _model_label(model_cfg["openai"]): lambda: _make_openai(
            api_keys["openai"], model_cfg["openai"],
            syl["temperature"], syl["max_tokens"], 240,
        ),
        _model_label(model_cfg["anthropic"]): lambda: ChatAnthropic(
            anthropic_api_key=api_keys["anthropic"],
            model=model_cfg["anthropic"],
            temperature=syl["temperature"],
            max_tokens=syl["max_tokens"],
            timeout=240,
            max_retries=2,
        ),
        _model_label(model_cfg["google"]): lambda: ChatGoogleGenerativeAI(
